...
|-------|------|-------------|
| `model_id` | `str` | Select the underlying model used by the agent. If you are using the default model_type (AnyLLM), you can refer to [AnyLLM Provider Docs](https://docs.mozilla.ai/any-llm/providers/) for the list of providers and how to access them. |
...
| `model_args` | `MutableMapping[str, Any] \| None` | Pass arguments to the model instance like `temperature`, `top_k`, as well as any other provider-specific parameters. Refer to [any-llm Completion API Docs](https://docs.mozilla.ai/any-llm/api/completion/) for more info. |
...
| `skills` | `list[a2a.types.AgentSkill] \| None` |  |
| `version` | `str` |  |
| `context_timeout_minutes` | `int` | Context timeout in minutes. Contexts will be cleaned up after this period of inactivity. |
| `history_formatter` | `Callable[list[AgentMessage], str, str]` | Function to format conversation history and current query into a single prompt. Takes (messages, current_query) and returns formatted string. |
| `task_cleanup_interval_minutes` | `int` | Interval in minutes between task cleanup runs. |
| `push_notifier_store_type` | `type[PushNotificationConfigStore]` |  |
| `push_notifier_sender_type` | `type[PushNotificationSender]` | Push notifier sender to be used by the agent. If not provided, a default async httpx-based push notifier sender will be used. |
| `task_store_type` | `type[TaskStore]` | Task store to be used by the agent. If not provided, a default in-memory task store will be used. |
| `stream_tool_usage` | `bool` | whether to stream tool execution results |
