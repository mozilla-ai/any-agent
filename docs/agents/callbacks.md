...
!!! note "Availability"
    ...
    The `get_messages()` and `set_messages()` methods are **only available in `before_llm_call` and `after_llm_call` callbacks**.

    - In `before_llm_call`: You can read and modify the messages that will be sent to the LLM
    - In `after_llm_call`: You can read and modify the messages including the LLM's response
    - In other callbacks (`before_tool_execution`, `after_tool_execution`, etc.): These methods will raise `NotImplementedError`
...
## Example: Modifying LLM responses

You can also modify the LLM's response after it's generated using the `after_llm_call` callback. This is useful for post-processing responses, adding disclaimers, or formatting output:

```python
from any_agent.callbacks.base import Callback
from any_agent.callbacks.context import Context

class AddDisclaimerCallback(Callback):
    def __init__(self, disclaimer: str):
    ...
    def after_llm_call(self, context: Context, *args, **kwargs) -> Context:
        try:
            ...
            if messages and len(messages) > 0:
                # The last message is the LLM's response
                last_message = messages[-1]
                if last_message.get("role") == "assistant":
                    # Append disclaimer to the LLM's response
...
callback = AddDisclaimerCallback(
