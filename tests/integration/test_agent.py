...
from any_agent.tracing.attributes import INPUT_MESSAGES, TOOL_ARGS
...
def assert_trace(agent_trace: AgentTrace, agent_framework: AgentFramework) -> None:
    def assert_first_llm_call(llm_call: AgentSpan) -> None:
        ...
        input_messages_raw = llm_call.attributes.get(INPUT_MESSAGES)
    ...
    def assert_first_tool_execution(tool_execution: AgentSpan) -> None:
        ...
        assert tool_execution.attributes.get(TOOL_ARGS, None) is not None
        # tool.args should be a JSON string (dict)
        tool_args_raw = tool_execution.attributes.get(TOOL_ARGS)
