...
def test_tool_error_creation() -> None:
...
def test_tool_error_string_representation() -> None:
...
def test_wrap_no_exception_with_sync_tool() -> None:
...
@pytest.mark.asyncio
async def test_wrap_no_exception_with_async_tool() -> None:
...
def test_span_generation_detects_tool_error() -> None:
...
def test_span_generation_backward_compatibility() -> None:
    ...
    assert isinstance(status1, Status)
...
def test_tool_error_in_set_tool_output() -> None:
    ...
    with patch("any_agent.callbacks.span_generation.base.Status"):
