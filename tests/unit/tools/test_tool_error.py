...
def test_span_generation_backward_compatibility():
    ...
    # Test "Error executing tool:" pattern  
    error2 = "Error executing tool: Network error"
    status2 = span_gen._determine_tool_status(error2, "text")
    assert status2.status_code == StatusCode.ERROR
    assert status2.description == error2
