...
async def mock_a2a_tool(agent_card, response):
    ...
    with patch("any_agent.tools.a2a.A2ACardResolver") as mock_resolver, \
        ...
        mock_client_instance.send_message.return_value = SendMessageResponse(root=response)
...
async def test_handles_task_response():
    ...
    async with mock_a2a_tool(mock_agent_card(), success_response) as (tool, mock_client):
